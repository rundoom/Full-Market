...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("ui_cancel") or (event.is_action_pressed("interaction") and is_shopping_avaliable and get_tree().paused):
	...
	elif event.is_action_pressed("interaction") and is_shopping_avaliable:
